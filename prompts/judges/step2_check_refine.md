...
| **细粒度**：大模块（>20文件）未拆分且子功能差异明显 | 40-65 | 否 |
| **细粒度**：大模块（>30文件）功能明显混杂未拆分 | 40-65 | 否 |
| **粗粒度**：模块内混有完全独立的顶层协议（文件数>15）且未拆分 | 40-60 | 否（欠拆） |
| **粗粒度**：同协议 client/server/config/utils 已拆成碎片模块 | 0 | 否（过度拆分）|
| **粗粒度**：文件数 ≤ 30 被拆分（即使功能有差异） | -20分惩罚 | — |
...
> ⚠️ **若 prompt 末尾存在「粒度约束」节，以该节为最高优先级，课分表中的粗粒度 / 细粒度专用行以该节为准。**
