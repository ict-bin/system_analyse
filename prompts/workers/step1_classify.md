...
2. **导入导出关系校正**：若 `details/*.json` 中显示某 ELF/SO 大量 NEEDED/导入另一个库，或 C/C++/Python 源码通过 include/import/call 依赖另一个文件/模块，说明它依赖后者；库文件与其直接使用者可合并或建立清晰上下游模块边界，不能只按文件名孤立分类。
...
**第五步（可选）：利用 ELF/SO 和源码依赖关系修正模块边界**
```bash
# details/<path>.json 中包含 symbols/imports/needed/source_imports
read details/path/to/libfoo.so.json
read details/path/to/source_file.c.json
read details/path/to/module.py.json
```
- `needed` 指向的库、头文件或 Python 模块通常是该文件依赖的下层能力。
- `symbols/exports/functions` 多且被其他文件 import/call/include 的文件通常是公共/底层模块。
- `imports` 多、依赖少的可执行文件/业务代码通常更接近系统外层入口。
