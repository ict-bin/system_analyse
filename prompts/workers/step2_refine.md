...
3. **依据依赖关系校正边界**：结合 `details/<path>.json` 中的 `symbols/imports/needed` 判断 ELF/SO 之间的上下游关系，避免把强依赖的一组文件错误拆散，或把无调用/依赖关系的文件强行合并
...
> ⚠️ **若 prompt 末尾存在「粒度约束」节，以该节规则为最高优先级，本节规则降级为参考。**
...
> **粗粒度模式**：上方基于文件数量的规则全部无效，具体规则见末尾「粒度约束」节。
