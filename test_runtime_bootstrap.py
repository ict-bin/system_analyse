...
class RuntimeBootstrapTests(unittest.IsolatedAsyncioTestCase):
    async def test_retries_db_init_until_success(self):
        ...
        self.assertFalse(status["worker_loop_ready"])
        self.assertTrue(status["provider_sync_done"])
        self.assertEqual(2, status["attempts"])
        self.assertEqual(2, len(init_attempts))

    async def test_runner_role_without_bootstrap_worker_loop_stays_not_ready(self):
        bootstrap = RuntimeBootstrap(
            pool_overrides=lambda svc: (1, 1, 1, 1),
            should_run_migrations=lambda: False,
        )
        ...
        with patch("app.service.runtime_bootstrap.get_service_yaml", return_value=SimpleNamespace(
            ...
            bootstrap,
            "_init_db",
            side_effect=lambda svc_yaml: setattr(bootstrap._status, "db_ready", True) or True,
        ):
            bootstrap.start(app)
            for _ in range(20):
                await asyncio.sleep(0.01)
        ...
        self.assertTrue(bootstrap.status()["db_ready"])
        self.assertFalse(bootstrap.status()["worker_loop_ready"])
        self.assertFalse(bootstrap.status()["ready"])
